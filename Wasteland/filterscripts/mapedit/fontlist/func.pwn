...
ApplyFontListCaption(playerid) {
    new materialindex = g_PlayerData[playerid][PLAYER_DATA_EDIT_OBJMATIDX];
    format(g_TextdrawString, sizeof g_TextdrawString, "Шрифт %i", materialindex);
    PlayerTextDrawSetString(playerid, g_FontListPTD[playerid][FONTLIST_PTD_CAPTION], g_TextdrawString);
}

ApplyFontListPage(playerid) {
    format(g_TextdrawString, sizeof g_TextdrawString, "Старница: %i / %i", g_FontListData[playerid][FONTLIST_DATA_PAGE] + 1, g_FontListData[playerid][FONTLIST_DATA_MAXPAGE] + 1);
...
ApplyFontListSearch(playerid) {
    ...
    if( isempty(g_SearchString) ) {
        PlayerTextDrawSetString(playerid, g_FontListPTD[playerid][FONTLIST_PTD_SEARCH], "Поиск");
    } else {
...
ShowFontListDialog(playerid, dialogid) {
    switch( dialogid ) {
        case DIALOGID_FONTLIST_PAGE: {
            format(g_DialogInfo, sizeof g_DialogInfo, "Страница: %i / %i", g_FontListData[playerid][FONTLIST_DATA_PAGE] + 1, g_FontListData[playerid][FONTLIST_DATA_MAXPAGE] + 1);
            ShowPlayerDialog(playerid, dialogid, DIALOG_STYLE_INPUT, "Список шрифтов: страница", g_DialogInfo, "Дальше", "Назад");
        }
        case DIALOGID_FONTLIST_SEARCH: {
            ...
            ShowPlayerDialog(playerid, dialogid, DIALOG_STYLE_INPUT, "Список шрифтов: Поиск", g_DialogInfo, "Дальше", "Назад");
        }
        default: {
            return 0;
        }
