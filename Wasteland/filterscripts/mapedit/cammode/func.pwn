...
ToggleCam(playerid, bool:toggle) {
    ...
    if( toggle ) {
        ...
        SendClientMessage(playerid, RGBA_ORANGE, "Управление Camera Mode: ");
        SendClientMessage(playerid, RGBA_ORANGE, "Управление: {FFFFFF}~k~~GO_FORWARD~ / ~k~~GO_BACK~ / ~k~~GO_LEFT~ / ~k~~GO_RIGHT~");
        SendClientMessage(playerid, RGBA_ORANGE, "Быстрее: {FFFFFF}~k~~PED_JUMPING~ + кнопки управления");
