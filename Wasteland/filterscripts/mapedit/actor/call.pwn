        ...
        switch(response) {
            case EDIT_RESPONSE_FINAL: {
                ...
                if( new_actorid == INVALID_ACTOR_ID ) {
                    SendClientMessage(playerid, RGBA_RED, "ERROR: Ётот актер не может быть перемещен / переспавнен!");
                }
    ...
    switch(dialogid) {
            ...
            switch(listitem) {
                ...
                case LISTITEM_ACTOR_MOVE: {
                    ...
                    if(objectid == INVALID_OBJECT_ID) {
                        SendClientMessage(playerid, RGBA_RED, "ERROR: Ётот актер не может быть перемещен сейчас!");
                    } else {
                        ...
                        SendClientMessage(playerid, RGBA_ORANGE, "–едактировать: {FFFFFF}Hold ~k~~PED_SPRINT~ to look around and press ESC to cancel.");
                        return 1;
                    }
                }
                ...
                case LISTITEM_ACTOR_DUPLICATE: {
                    ...
                    if( new_actorid == INVALID_ACTOR_ID ) {
                        SendClientMessage(playerid, RGBA_RED, "ERROR: ¬ы не можете создать копию этого актера!");
                    } else {
        ...
        case DIALOGID_ACTOR_ANIM_DELTA: {
            ...
            if(sscanf(inputtext, "f", delta)) {
                SendClientMessage(playerid, RGBA_RED, "ERROR: ¬ы не ввели значенние!");
            } else if(delta < 0) {
                SendClientMessage(playerid, RGBA_RED, "ERROR: «начение не можеть быть негативным!");
            } else {
        ...
        case DIALOGID_ACTOR_ANIM_TIME: {
            ...
            if(sscanf(inputtext, "i", time)) {
                SendClientMessage(playerid, RGBA_RED, "ERROR: “олько цифры!");
            } else if(time < 0 ) {
                SendClientMessage(playerid, RGBA_RED, "ERROR: «начение не можеть быть негативным!");
            } else {
