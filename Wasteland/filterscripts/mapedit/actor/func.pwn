ShowActorDialog(playerid, dialogid) {
    switch( dialogid ) {
            ...
            for(new listitem; listitem < MAX_LISTITEMS_ACTOR; listitem ++) {
                switch(listitem) {
                    ...
                    case LISTITEM_ACTOR_COMMENT_RESET: {
                        ...
                        format(g_DialogInfoRow, sizeof g_DialogInfoRow, "Улалить комментарий к\t%s\n", g_SkinString);
                        strcat(g_DialogInfo, g_DialogInfoRow);
                    }
        ...
        case DIALOGID_ACTOR_COORD: {
            ...
            ShowPlayerDialog(playerid, dialogid, DIALOG_STYLE_INPUT, "Координаты", g_DialogInfo, "Enter", "Cancel");
        }
        case DIALOGID_ACTOR_COMMENT: {
            new actorid = GetPlayerEditActor(playerid);
            strunpack(g_CommentString, g_ActorData[actorid][ACTOR_DATA_COMMENT], MAX_COMMENT_LEN+1);
            format(g_DialogInfo, sizeof g_DialogInfo, "Текущий комментарий: %s", g_CommentString);
            ShowPlayerDialog(playerid, dialogid, DIALOG_STYLE_INPUT, "Комментарий", g_DialogInfo, "Enter", "Cancel");
        }
        case DIALOGID_ACTOR_ANIM_DELTA: {
            new actorid = GetPlayerEditActor(playerid);
            format(g_DialogInfo, sizeof g_DialogInfo, "Значение: %f", g_ActorData[actorid][ACTOR_DATA_ANIM_DELTA] );
        ...
        case DIALOGID_ACTOR_ANIM_TIME: {
            new actorid = GetPlayerEditActor(playerid);
            format(g_DialogInfo, sizeof g_DialogInfo, "Значение: %i", g_ActorData[actorid][ACTOR_DATA_ANIM_TIME] );
