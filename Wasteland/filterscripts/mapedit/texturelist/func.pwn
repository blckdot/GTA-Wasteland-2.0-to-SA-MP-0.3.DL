...
ApplyTextureListCaption(playerid) {
    format(g_TextdrawString, sizeof g_TextdrawString, "Индекс тектуры %i", g_PlayerData[playerid][PLAYER_DATA_EDIT_OBJMATIDX]);
    PlayerTextDrawSetString(playerid, g_TextureListPTD[playerid][TEXTURELIST_PTD_CAPTION], g_TextdrawString);
}

ApplyTextureListPage(playerid) {
    format(g_TextdrawString, sizeof g_TextdrawString, "Страница %i / %i", g_TextureListData[playerid][TEXTURELIST_DATA_PAGE] + 1, g_TextureListData[playerid][TEXTURELIST_DATA_MAXPAGE] + 1);
...
ApplyTextureListCategory(playerid) {
    ...
    if( categoryid == INVALID_CATEGORY_ID ) {
        PlayerTextDrawSetString(playerid, g_TextureListPTD[playerid][TEXTURELIST_PTD_CATEGORY], "Категория");
    } else {
...
ApplyTextureListSearch(playerid) {
    ...
    if( isempty(g_SearchString) ) {
        PlayerTextDrawSetString(playerid, g_TextureListPTD[playerid][TEXTURELIST_PTD_SEARCH], "Поиск");
    } else {
...
ShowTextureListDialog(playerid, dialogid) {
    switch( dialogid ) {
        case DIALOGID_TEXTURELIST_PAGE: {
            format(g_DialogInfo, sizeof g_DialogInfo, "Страница: %i / %i", g_TextureListData[playerid][TEXTURELIST_DATA_PAGE] + 1, g_TextureListData[playerid][TEXTURELIST_DATA_MAXPAGE] + 1);
            ShowPlayerDialog(playerid, dialogid, DIALOG_STYLE_INPUT, "Список текстур: страница", g_DialogInfo, "Выбор", "Назад");
        }
        case DIALOGID_TEXTURELIST_SEARCH: {
            ...
            ShowPlayerDialog(playerid, dialogid, DIALOG_STYLE_INPUT, "Список текстур: Поиск", g_DialogInfo, "Выбор", "Назад");
        }
        default: {
            return 0;
        }
