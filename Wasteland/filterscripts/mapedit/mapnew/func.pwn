ShowMapNewDialog(playerid, dialogid) {
    switch( dialogid ) {
        case DIALOGID_MAP_NEW: {
            format(g_DialogInfo, sizeof g_DialogInfo, "Введите \"%s\" чтобы создать новую карту.", NEWMAP_COMMAND);
            ShowPlayerDialog(playerid, dialogid, DIALOG_STYLE_INPUT, "Новая карта", g_DialogInfo, "Создать", "Отмена");
        }
        default: {
            return 0;
        }
