...
ShowMapLoadDialog(playerid, dialogid) {
    switch( dialogid ) {
        case DIALOGID_MAP_LOAD: {
            ShowPlayerDialog(playerid, dialogid, DIALOG_STYLE_INPUT, "Загрузка карты", "Введите название карты которую вы хотите загрузить:", "Загрузить", "Отмена");
        }
...
MapLoad(
    ...
    while( fread(file_handle, buffer) ) {
        ...
        if( sscanf(buffer, "p<(>s[100]p<;>s[300]S(без комментария)[100]", func, params, comment) ) {
            continue;
        }
        ...
        if( isempty(comment) ) {
            format(comment, sizeof comment, "без комментария");
        }
