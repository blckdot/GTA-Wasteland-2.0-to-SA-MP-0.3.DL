...
GetCreateListCaption(playerid, caption[], caption_size) {
    switch( g_PlayerData[playerid][PLAYER_DATA_TDMODE] ) {
        case TDMODE_CREATELIST_OBJECT: {
            format(caption, caption_size, "Создать объект");
        }
        case TDMODE_CREATELIST_VEHICLE: {
            format(caption, caption_size, "Создать транспорт");
        }
        case TDMODE_CREATELIST_PICKUP: {
            format(caption, caption_size, "Создать пикап");
        }
        case TDMODE_CREATELIST_ACTOR: {
            format(caption, caption_size, "Создать актера");
        }
        case TDMODE_CREATELIST_ATTACH: {
            format(caption, caption_size, "Создать аттач");
        }
        default: {
            return format(caption, caption_size, "Создать"), 0;
        }
    }
    return 1;
}
...
ApplyCreateListPage(playerid) {
    format(g_TextdrawString, sizeof g_TextdrawString, "Страница: %i / %i", GetCreateListPage(playerid) + 1, GetCreateListMaxPage(playerid) + 1);
...
ApplyCreateListCategory(playerid) {
    ...
    if( categoryid == INVALID_CATEGORY_ID ) {
        PlayerTextDrawSetString(playerid, g_CreateListPTD[playerid][CREATELIST_PTD_CATEGORY], "Категория");
        return 1;
    }
    ...
    format(g_TextdrawString, sizeof g_TextdrawString, "Категория: %s", g_CategoryNameString);
...
ApplyCreateListSearch(playerid) {
    ...
    if( isempty(g_SearchString) ) {
        PlayerTextDrawSetString(playerid, g_CreateListPTD[playerid][CREATELIST_PTD_SEARCH], "Поиск");
    } else {
...
ShowCreateListDialog(playerid, dialogid) {
    switch( dialogid ) {
        case DIALOGID_CREATELIST_PAGE: {
            ...
            format(g_DialogInfo, sizeof g_DialogInfo, "Страница: %i / %i", GetCreateListPage(playerid) + 1, GetCreateListMaxPage(playerid) + 1);
            ShowPlayerDialog(playerid, dialogid, DIALOG_STYLE_INPUT, g_DialogCaption, g_DialogInfo, "Дальше", "Отмена");
        }
