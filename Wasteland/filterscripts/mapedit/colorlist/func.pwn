...
ApplyColorListCaption(playerid) {
    switch( g_PlayerData[playerid][PLAYER_DATA_TDMODE] ) {
        case TDMODE_COLORLIST_TEXTURE: {
            new materialindex = g_PlayerData[playerid][PLAYER_DATA_EDIT_OBJMATIDX];
            format(g_TextdrawString, sizeof g_TextdrawString, "Цвет текстуры %i", materialindex);
            PlayerTextDrawSetString(playerid, g_ColorListPTD[playerid][COLORLIST_PTD_CAPTION], g_TextdrawString);
        }
        case TDMODE_COLORLIST_FONTFACE: {
            new materialindex = g_PlayerData[playerid][PLAYER_DATA_EDIT_OBJMATIDX];
            format(g_TextdrawString, sizeof g_TextdrawString, "Цвет текста %i", materialindex);
            PlayerTextDrawSetString(playerid, g_ColorListPTD[playerid][COLORLIST_PTD_CAPTION], g_TextdrawString);
        }
        case TDMODE_COLORLIST_FONTBACK: {
            new materialindex = g_PlayerData[playerid][PLAYER_DATA_EDIT_OBJMATIDX];
            format(g_TextdrawString, sizeof g_TextdrawString, "Цвет фона %i", materialindex);
            PlayerTextDrawSetString(playerid, g_ColorListPTD[playerid][COLORLIST_PTD_CAPTION], g_TextdrawString);
        }
        case TDMODE_COLORLIST_ATTACH_1: {
            PlayerTextDrawSetString(playerid, g_ColorListPTD[playerid][COLORLIST_PTD_CAPTION], "Цвет объекта [attach] 1");
        }
        case TDMODE_COLORLIST_ATTACH_2: {
            PlayerTextDrawSetString(playerid, g_ColorListPTD[playerid][COLORLIST_PTD_CAPTION], "Цвет объекта [attach] 2");
        }
        case TDMODE_COLORLIST_VEHICLE_1: {
            PlayerTextDrawSetString(playerid, g_ColorListPTD[playerid][COLORLIST_PTD_CAPTION], "Цвет машины 1");
        }
        case TDMODE_COLORLIST_VEHICLE_2: {
            PlayerTextDrawSetString(playerid, g_ColorListPTD[playerid][COLORLIST_PTD_CAPTION], "Цвет машины 2");
        }
        default: {
            return 0;
        }
...
ApplyColorListPage(playerid) {
    format(g_TextdrawString, sizeof g_TextdrawString, "Страница: %i / %i", GetColorListPage(playerid) + 1, GetColorListMaxPage(playerid) + 1);
...
ApplyColorListSearch(playerid) {
    ...
    if( isempty(g_SearchString) ) {
        PlayerTextDrawSetString(playerid, g_ColorListPTD[playerid][COLORLIST_PTD_SEARCH], "Поиск");
    } else {
...
ApplyColorListAppliedRow(playerid) {
    ...
    CreatePlayerTextDraw          (playerid, 184.0, y, "Цвет установлен");
...
ShowColorListDialog(playerid, dialogid) {
	switch( dialogid ) {
        case DIALOGID_COLORLIST_PAGE: {
            format(g_DialogInfo, sizeof g_DialogInfo, "Страница: %i / %i", GetColorListPage(playerid) + 1, GetColorListMaxPage(playerid) + 1);
            ShowPlayerDialog(playerid, dialogid, DIALOG_STYLE_INPUT, "Список цветов: страница", g_DialogInfo, "Дальше", "Назад");
        }
        case DIALOGID_COLORLIST_SEARCH: {
            ...
            ShowPlayerDialog(playerid, dialogid, DIALOG_STYLE_INPUT, "Список цветов: поиск", g_DialogInfo, "Дальше", "Назад");
        }
        default: {
            return 0;
        }
