...
ToggleOffsetEdit(playerid, bool:toggle) {
    ...
    if(toggle) {
        ...
        SendClientMessage(playerid, RGBA_ORANGE, "Кномки редактирования:");
        SendClientMessage(playerid, RGBA_ORANGE, "Управление: {FFFFFF}+~k~~VEHICLE_TURRETRIGHT~ / -~k~~VEHICLE_TURRETLEFT~");
        SendClientMessage(playerid, RGBA_ORANGE, "Медленее: {FFFFFF}~k~~SNEAK_ABOUT~ + кнопки управления");
        SendClientMessage(playerid, RGBA_ORANGE, "Изменить мод: {FFFFFF}~k~~PED_SPRINT~ + кнопки управления");
