        ...
        switch(response) {
            case EDIT_RESPONSE_FINAL: {
                ...
                if( RecreatePickup(pickupid) == INVALID_PICKUP_ID) {
                    SendClientMessage(playerid, RGBA_RED, "ERROR: Вы не можете двигать этот пикап");
                }
...
public OnDialogResponse(playerid, dialogid, response, listitem, inputtext[]) {
    switch(dialogid) {
            ...
            switch(listitem) {
                ...
                case LISTITEM_PICKUP_MOVE: {
                    ...
                    if(objectid == INVALID_OBJECT_ID) {
                        SendClientMessage(playerid, RGBA_RED, "ERROR: Вы не можете двигать этот пикап сейчас!");
                    } else {
                        ...
                        SendClientMessage(playerid, RGBA_ORANGE, "Редактирование: {FFFFFF}Удериживайте ~k~~PED_SPRINT~ чтобы отсмотрется по сторонам и ESC для отмены.");
                        return 1;
                    }
                }
                ...
                case LISTITEM_PICKUP_DUPLICATE: {
                    ...
                    if( new_pickupid == INVALID_PICKUP_ID ) {
                        SendClientMessage(playerid, RGBA_RED, "ERROR: Вы не может создать копию этого пикапа!");
                    } else {
        ...
        case DIALOGID_PICKUP_COORD: {
            ...
            new new_pickupid = RecreatePickup(pickupid);
            if( new_pickupid == INVALID_PICKUP_ID) {
                SendClientMessage(playerid, RGBA_RED, "ERROR: Вы не можете двигать этотт пикап");
            }
