            ...
            switch(listitem) {
                ...
                case LISTITEM_VEHICLE_DRIVE: {
                    if( IsVehicleOccupied(vehicleid, 0) ) {
                        SendClientMessage(playerid, RGBA_RED, "ERROR: Кто-то уже сидит в этом автомобиле!");
                    } else if( GetPlayerState(playerid) == PLAYER_STATE_SPECTATING ) {
                        SendClientMessage(playerid, RGBA_RED, "ERROR: Вы должны заспавнится для начала(выйдите из flymode)!");
                    } else {
                ...
                case LISTITEM_VEHICLE_MOVE: {
                    ...
                    if(objectid == INVALID_OBJECT_ID) {
                        SendClientMessage(playerid, RGBA_RED, "ERROR: Вы не можете передвигать этот автомобиль сейчас!");
                    } else {
                        ...
                        SendClientMessage(playerid, RGBA_ORANGE, "Редактирование: {FFFFFF}Удерживайте ~k~~PED_SPRINT~ чтобы посмотреть по сторонам или ESC для отмены.");
                        return 1;
                    }
                }
                ...
                case LISTITEM_VEHICLE_DUPLICATE: {
                    ...
                    if( new_vehicleid == INVALID_VEHICLE_ID ) {
                        SendClientMessage(playerid, RGBA_RED, "ERROR: Вы не можеть клонировать этот автомобиль!");
                    } else {
                        if( !CopyVehicleAttachments(.from_vehicleid = vehicleid, .to_vehicleid = new_vehicleid) ) {
                            SendClientMessage(playerid, RGBA_RED, "WARNING: Все аттачи к автомобилю не могут быть клонированы!");
                        }
                ...
                case LISTITEM_VEHICLE_RECREATE: {
                    ...
                    if( new_vehicleid == INVALID_VEHICLE_ID ) {
                        SendClientMessage(playerid, RGBA_RED, "ERROR: Вы не можете пересоздать этот автомобиль!");
                    }
                }
                ...
                case LISTITEM_VEHICLE_MODSHOP: {
                    ...
                    if( GetPlayerVehicleID(playerid) != vehicleid || GetPlayerVehicleSeat(playerid) != 0 ) {
                        if( IsVehicleOccupied(vehicleid, 0) ) {
                            SendClientMessage(playerid, RGBA_RED, "ERROR: Кто-то уже сидит в транспорте!");
                            return ShowVehicleDialog(playerid, dialogid), 1;
                        } else {
                            PutPlayerInVehicle(playerid, vehicleid, 0);
                        }
                ...
                case LISTITEM_VEHICLE_REMOVEMODS: {
                    ...
                    if(components_removed == 0) {
                        SendClientMessage(playerid, RGBA_RED, "ERROR: У этогоо автомобиля нет модификаций!");
                    }
                }
            }
