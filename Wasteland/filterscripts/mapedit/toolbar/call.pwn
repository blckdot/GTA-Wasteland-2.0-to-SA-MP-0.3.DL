...
public OnPlayerClickTextDraw(playerid, Text:clickedid) {
    ...
    if( clickedid == g_ToolbarTextdraw[TD_TOOLBAR_OBJECT_NSELECT] ) {
        ...
        if( objectid == INVALID_OBJECT_ID ) {
            return SendClientMessage(playerid, RGBA_RED, "ERROR: В зоне стриминга нет объектов!"), 1;
        }
    ...
    if( clickedid == g_ToolbarTextdraw[TD_TOOLBAR_OBJECT_LOOKSELECT] ) {
        ...
        if( objectid == INVALID_OBJECT_ID ) {
            return SendClientMessage(playerid, RGBA_RED, "ERROR: Вы не смотрите на объект!"), 1;
        }
    ...
    if( clickedid == g_ToolbarTextdraw[TD_TOOLBAR_OBJECT_SSELECT] ) {
        ...
        if( objectid == INVALID_OBJECT_ID ) {
            return SendClientMessage(playerid, RGBA_RED, "ERROR: Вы не стоите на объекте(он должен быть в движении)!"), 1;
        }
    ...
    if( clickedid == g_ToolbarTextdraw[TD_TOOLBAR_OBJECT_3DSELECT] ) {
        ...
        SendClientMessage(playerid, RGBA_ORANGE, "3D-выбор объекта: {FFFFFF}Удерживайте ~k~~PED_SPRINT~ чтобы посмотреть по сторонам или ESC для отмены.");
        return 1;
    }
    ...
    if( clickedid == g_ToolbarTextdraw[TD_TOOLBAR_VEHICLE_NSELECT] ) {
        ...
        if( vehicleid == INVALID_VEHICLE_ID ) {
            return SendClientMessage(playerid, RGBA_RED, "ERROR: Вы не внутри автомобиля!"), 1;
        }
    ...
    if( clickedid == g_ToolbarTextdraw[TD_TOOLBAR_VEHICLE_LOOKSELECT] ) {
        ...
        if( vehicleid == INVALID_VEHICLE_ID ) {
            return SendClientMessage(playerid, RGBA_RED, "ERROR: Вы не смотрите на автомобиль!"), 1;
        }
    ...
    if( clickedid == g_ToolbarTextdraw[TD_TOOLBAR_VEHICLE_SSELECT] ) {
        ...
        if( vehicleid == INVALID_VEHICLE_ID ) {
            return SendClientMessage(playerid, RGBA_RED, "ERROR: Вы не стоите на автомобиле(он должен быть в движении)!"), 1;
        }
    ...
    if( clickedid == g_ToolbarTextdraw[TD_TOOLBAR_PICKUP_NSELECT] ) {
        ...
        if( pickupid == INVALID_PICKUP_ID ) {
            return SendClientMessage(playerid, RGBA_RED, "ERROR: В зоне стриминга нет пикапов!"), 1;
        }
    ...
    if( clickedid == g_ToolbarTextdraw[TD_TOOLBAR_ACTOR_NSELECT] ) {
        ...
        if( actorid == INVALID_ACTOR_ID ) {
            return SendClientMessage(playerid, RGBA_RED, "ERROR: Вы не рядом в актером!"), 1;
        }
    ...
    if( clickedid == g_ToolbarTextdraw[TD_TOOLBAR_ACTOR_LOOKSELECT] ) {
        ...
        if( actorid == INVALID_ACTOR_ID ) {
            return SendClientMessage(playerid, RGBA_RED, "ERROR: Вы не смотрите на актера!"), 1;
        }
