ShowMapSaveDialog(playerid, dialogid) {
    switch( dialogid ) {
        case DIALOGID_MAP_SAVE_AS: {
            ShowPlayerDialog(playerid, dialogid, DIALOG_STYLE_INPUT, "Сохранение карты", "Введите название для вашей карты:", "Сохранить", "Отмена");
        }
        case DIALOGID_MAP_SAVE_CONFIRM: {
            format(g_DialogInfo, sizeof g_DialogInfo, "Введите \"%s\" чтобы сохранить карту.", SAVEMAP_COMMAND);
            ShowPlayerDialog(playerid, dialogid, DIALOG_STYLE_INPUT, "Сохрание карты", g_DialogInfo, "Сохранить", "Отмена");
        }
        default: {
            return 0;
        }
