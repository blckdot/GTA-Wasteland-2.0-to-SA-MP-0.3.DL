...
public OnDialogResponse(playerid, dialogid, response, listitem, inputtext[]) {
    switch( dialogid ) {
        case DIALOGID_CATMANAGER_CAT_CREATE: {
            ...
            if( create_success ) {
                SendClientMessage(playerid, RGBA_GREEN, "Категория создана успешно.");
            } else {
                SendClientMessage(playerid, RGBA_RED, "ERROR: Категория не может быть создана!");
                ShowCategoryManagerDialog(playerid, dialogid);
            }
            return 1;
        }
        case DIALOGID_CATMANAGER_CAT_DESTROY: {
            ...
            if( isempty(inputtext) || strcmp(inputtext, CATMANAGER_DELETE_CODE, true) != 0 ) {
                SendClientMessage(playerid, RGBA_RED, "ERROR: Неверный код!");
                ShowCategoryManagerDialog(playerid, dialogid);
                return 1;
            }
            ...
            if( success ) {
                SendClientMessage(playerid, RGBA_GREEN, "Категория удалена успешно.");
                ...
                SendClientMessage(playerid, RGBA_RED, "ERROR: Категория не может быть удалена!");
                ShowCategoryManagerDialog(playerid, dialogid);
            }
            return 1;
        }
        case DIALOGID_CATMANAGER_CAT_RENAME: {
            ...
            if( success ) {
                SendClientMessage(playerid, RGBA_GREEN, "Категория переименована успешно.");

                ApplyCategoryManagerCatName(playerid);
            } else {
                SendClientMessage(playerid, RGBA_RED, "ERROR: Категория не может быть переименована!");
                ShowCategoryManagerDialog(playerid, dialogid);
            }
            return 1;
        }
        case DIALOGID_CATMANAGER_PAGE_ALL, DIALOGID_CATMANAGER_PAGE_CAT: {
            ...
            if( sscanf(inputtext, "i", page) || (page-1) < MIN_CATMANAGER_PAGE || (page-1) > maxpage ) {
                SendClientMessage(playerid, RGBA_RED, "ERROR: Неверный номер страницы!");
                ShowCategoryManagerDialog(playerid, dialogid);
                return 1;
            }
    ...
    for(new row; row < MAX_CATMANAGER_ROWS; row ++) {
        if(
            ...
            if( bind_success ) {
                ...
                format(g_ClientMessage, sizeof g_ClientMessage, "ERROR: Этот объект не можеть быть %s категорию!", create_bind ? ("добавлен в") : ("удален из"));
