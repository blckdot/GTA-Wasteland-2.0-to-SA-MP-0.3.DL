...
ShowObjectDialog(playerid, dialogid) {
    switch( dialogid ) {
        case DIALOGID_OBJECT_MAIN: {
            ...
            for(new listitem; listitem < MAX_LISTITEMS_OBJECT; listitem ++) {
                switch(listitem) {
                    case LISTITEM_OBJECT_GOTO: {
                        strcat(g_DialogInfo, "Телепортироватся\t \n");
                    }
                    case LISTITEM_OBJECT_GET: {
                        strcat(g_DialogInfo, "Телепортировать к себе\t \n");
                    }
                    case LISTITEM_OBJECT_COORD: {
                        if( g_ObjectData[objectid-1][OBJECT_DATA_ATTACH_IDTYPE] == ID_TYPE_NONE ) {
                            strcat(g_DialogInfo, "Координаты\t \n");
                        } else {
                            strcat(g_DialogInfo, "Перемещение по координатам\t \n");
                        }
                    }
                    case LISTITEM_OBJECT_MOVE: {
                        if( g_ObjectData[objectid-1][OBJECT_DATA_ATTACH_IDTYPE] == ID_TYPE_NONE ) {
                            strcat(g_DialogInfo, "Редактировать\t \n");
                        } else {
                            strcat(g_DialogInfo, "Включить Перемещение по координатам\t \n");
                        }
                    }
                    case LISTITEM_OBJECT_ATTACH_SELECT: {
                        strcat(g_DialogInfo, "Выбрать как аттач\t \n");
                    }
                    case LISTITEM_OBJECT_ATTACH_APPLY: {
                        ...
                        if( !IsValidObject(attachobject) || attachobject == objectid ) {
                            ...
                            format(g_DialogInfoRow, sizeof g_DialogInfoRow, "Прикрепить выбраный объект\t%s\n", g_CommentString);
                    ...
                    case LISTITEM_OBJECT_ATTACH_REMOVE: {
                        switch( g_ObjectData[objectid-1][OBJECT_DATA_ATTACH_IDTYPE] ) {
                            case ID_TYPE_OBJECT: {
                                ...
                                strunpack(g_CommentString, g_ObjectData[attachobject-1][OBJECT_DATA_COMMENT], MAX_COMMENT_LEN+1);
                                format(g_DialogInfoRow, sizeof g_DialogInfoRow, "Открепить от объекта\t%s\n", g_CommentString );
                                strcat(g_DialogInfo, g_DialogInfoRow);
                            }
                            case ID_TYPE_VEHICLE: {
                                ...
                                format(g_DialogInfoRow, sizeof g_DialogInfoRow, "Открепить от автомобиля\t%s\n", g_CommentString );
                                strcat(g_DialogInfo, g_DialogInfoRow);
                            }
                            default: {
                                strcat(g_DialogInfo, "Попытатся открепить\t \n");
                            }
                        }
                    }
                    case LISTITEM_OBJECT_MATERIALS_COPY: {
                        strcat(g_DialogInfo, "Установить текстуру\t \n");
                    }
                        ...
                        switch( g_ObjectData[objectid-1][OBJECT_DATA_MATINDEX_TYPE][materialindex] ) {
                            case MATERIALINDEX_TYPE_TEXTURE: {
                                ...
                                if( textureid == INVALID_TEXTURE_ID ) {
                                    format(g_DialogInfoRow, sizeof g_DialogInfoRow, "Индекс тектуры %i\t{%06x}Цвет\n", materialindex, color_rgb);
                                } else {
                                    new modelid;
                                    GetTextureData(textureid, modelid, g_TextureTXDString, sizeof g_TextureTXDString, g_TextureNameString, sizeof g_TextureNameString);
                                    format(g_DialogInfoRow, sizeof g_DialogInfoRow, "Индекс тектуры %i\t{%06x}%i %s %s\n", materialindex, color_rgb, modelid, g_TextureTXDString, g_TextureNameString);
                                }
                            }
                            case MATERIALINDEX_TYPE_TEXT: {
                                new
                                ...
                                format(g_DialogInfoRow, sizeof g_DialogInfoRow, "Индекс тектуры %i\t{%06x}%s\n", materialindex, color_rgb, g_ObjectTextString);
                            }
                            default: {
                                format(g_DialogInfoRow, sizeof g_DialogInfoRow, "Индекс тектуры %i\tСтандартный / Неизвестный\n", materialindex);
                            }
                        }
                        strcat(g_DialogInfo, g_DialogInfoRow);
            ...
            ShowPlayerDialog(playerid, dialogid, DIALOG_STYLE_TABLIST, "Объект", g_DialogInfo, "Выбор", "Отмена");
        }
        case DIALOGID_OBJECT_INDEX: {
            ...
            format(g_DialogCaption, sizeof g_DialogCaption, "Индекс текстуры: %i", materialindex);
            g_DialogInfo = "";

            for(new listitem; listitem < MAX_LISTITEMS_OINDEX; listitem ++) {
                switch(listitem) {
                    case LISTITEM_OINDEX_REMOVE: {
                        strcat(g_DialogInfo, "Все\tВостановить индексы текстур\t-\n");
                    }
                    case LISTITEM_OINDEX_MAT: {
                        if( indextype == MATERIALINDEX_TYPE_TEXTURE ) {
                            ...
                            format(g_DialogInfoRow, sizeof g_DialogInfoRow, "Тектура\tТектура\t%i %s %s\n", modelid, g_TextureTXDString, g_TextureNameString);
                            strcat(g_DialogInfo, g_DialogInfoRow);
                        } else {
                            strcat(g_DialogInfo, "Тектура\tТектура\t-\n");
                        }
                    }
                    case LISTITEM_OINDEX_MAT_COLOR_S: {
                        if( indextype == MATERIALINDEX_TYPE_TEXTURE ) {
                            new argb = g_ObjectData[objectid-1][OBJECT_DATA_MATINDEX_COLOR][materialindex];
                            format(g_DialogInfoRow, sizeof g_DialogInfoRow, "Тектура\tЦвет\t{%06x}Цвет\n", ARGBtoRGB(argb) );
                            strcat(g_DialogInfo, g_DialogInfoRow);
                        } else {
                            strcat(g_DialogInfo, "Тектура\tЦвет\t-\n");
                        }
                    }
                    case LISTITEM_OINDEX_MAT_COLOR_A: {
                        if( indextype == MATERIALINDEX_TYPE_TEXTURE ) {
                            ...
                            format(g_DialogInfoRow, sizeof g_DialogInfoRow, "Тектура\tAlpha\t%i/%i\n", ARGBtoA(argb), 0xFF );
                            strcat(g_DialogInfo, g_DialogInfoRow);
                        } else {
                            strcat(g_DialogInfo, "Тектура\tAlpha\t-\n");
                        }
                    }
                    case LISTITEM_OINDEX_MAT_COLOR_R: {
                        strcat(g_DialogInfo, "Тектура\tВостановить цвет\t-\n");
                    }
                    case LISTITEM_OINDEX_TXT: {
                        if( indextype == MATERIALINDEX_TYPE_TEXT ) {
                            strunpack(g_ObjectTextString, g_ObjectText[objectid-1][materialindex], MAX_OBJECT_TEXT+1);
                            format(g_DialogInfoRow, sizeof g_DialogInfoRow, "Текст\tСтрока\t%s\n", g_ObjectTextString );
                            strcat(g_DialogInfo, g_DialogInfoRow);
                        } else {
                            strcat(g_DialogInfo, "Текст\tСтрока\t-\n");
                        }
                    }
                    case LISTITEM_OINDEX_TXT_MATSIZE: {
                        if( indextype == MATERIALINDEX_TYPE_TEXT ) {
                            ...
                            format(g_DialogInfoRow, sizeof g_DialogInfoRow, "Текст\tРазмер\t%s\n", g_MaterialSizeString);
                            strcat(g_DialogInfo, g_DialogInfoRow);
                        } else {
                            strcat(g_DialogInfo, "Текст\tРазмер\t-\n");
                        }
                    }
                    case LISTITEM_OINDEX_TXT_FONTFACE: {
                        if(indextype == MATERIALINDEX_TYPE_TEXT) {
                            strunpack(g_FontString, g_ObjectFont[objectid-1][materialindex], MAX_FONTNAME_LEN+1);
                            format(g_DialogInfoRow, sizeof g_DialogInfoRow, "Текст\tFont Face\t%s\n", g_FontString );
                            strcat(g_DialogInfo, g_DialogInfoRow);
                        } else {
                            strcat(g_DialogInfo, "Текст\tFont Face\t-\n");
                        }
                    }
                    case LISTITEM_OINDEX_TXT_FONTSIZE: {
                        if(indextype == MATERIALINDEX_TYPE_TEXT) {
                            new fontsize = g_ObjectData[objectid-1][OBJECT_DATA_MATINDEX_FONTSIZE][materialindex];
                            format(g_DialogInfoRow, sizeof g_DialogInfoRow, "Текст\tРазмер\t%i\n", fontsize );
                            strcat(g_DialogInfo, g_DialogInfoRow);
                        } else {
                            strcat(g_DialogInfo, "Текст\tРазмер\t-\n");
                        }
                    ...
                    case LISTITEM_OINDEX_TXT_ISBOLD: {
                        if(indextype == MATERIALINDEX_TYPE_TEXT) {
                            format(g_DialogInfoRow, sizeof g_DialogInfoRow, "Текст\tЖирный шрифт\t%s\n", g_ObjectData[objectid-1][OBJECT_DATA_MATINDEX_ISBOLD][materialindex] ? ("Yes") : ("No") );
                            strcat(g_DialogInfo, g_DialogInfoRow);
                        } else {
                            strcat(g_DialogInfo, "Текст\tЖирный шрифт\t-\n");
                        }
                    }
                    case LISTITEM_OINDEX_TXT_ALIGN: {
                        if(indextype == MATERIALINDEX_TYPE_TEXT) {
                            ...
                            format(g_DialogInfoRow, sizeof g_DialogInfoRow, "Текст\tПоложение\t%s\n", g_MaterialAlignString );
                            strcat(g_DialogInfo, g_DialogInfoRow);
                        } else {
                            strcat(g_DialogInfo, "Текст\tПоложение\t-\n");
                        }
                    }
                    case LISTITEM_OINDEX_TXT_FONTCOLOR_S: {
                        if(indextype == MATERIALINDEX_TYPE_TEXT) {
                            new fontcolor = g_ObjectData[objectid-1][OBJECT_DATA_MATINDEX_FONTCOLOR][materialindex];
                            format(g_DialogInfoRow, sizeof g_DialogInfoRow, "Текст\tЦвет\t{%06x}Цвет\n", ARGBtoRGB( fontcolor ) );
                            strcat(g_DialogInfo, g_DialogInfoRow);
                        } else {
                            strcat(g_DialogInfo, "Текст\tЦвет\t-\n");
                        }
                    }
                    case LISTITEM_OINDEX_TXT_FONTCOLOR_A: {
                        if(indextype == MATERIALINDEX_TYPE_TEXT) {
                            new fontcolor = g_ObjectData[objectid-1][OBJECT_DATA_MATINDEX_FONTCOLOR][materialindex];
                            format(g_DialogInfoRow, sizeof g_DialogInfoRow, "Текст\tFont Alpha\t%i/%i\n", ARGBtoA( fontcolor ), 0xFF);
                            strcat(g_DialogInfo, g_DialogInfoRow);
                        } else {
                            strcat(g_DialogInfo, "Текст\tFont Alpha\t-\n");
                        }
                     }
                    case LISTITEM_OINDEX_TXT_BACKCOLOR_S: {
                        if( indextype == MATERIALINDEX_TYPE_TEXT ) {
                            new argb = g_ObjectData[objectid-1][OBJECT_DATA_MATINDEX_COLOR][materialindex];
                            format(g_DialogInfoRow, sizeof g_DialogInfoRow, "Текст\tФон\t{%06x}Цвет\n", ARGBtoRGB( argb ) );
                            strcat(g_DialogInfo, g_DialogInfoRow);
                        } else {
                            strcat(g_DialogInfo, "Текст\tФон\t-\n");
                        }
                    }
                    case LISTITEM_OINDEX_TXT_BACKCOLOR_O: {
                        strcat(g_DialogInfo, "Текст\tНепрозрачный фон\t-\n");
                    }
                    case LISTITEM_OINDEX_TXT_BACKCOLOR_T: {
                        strcat(g_DialogInfo, "Текст\tПрозрачный фон\t-\n");
                     }
                    default: {
                        strcat(g_DialogInfo, "-\t-\t-\n");
                    }
            ...
            ShowPlayerDialog(playerid, dialogid, DIALOG_STYLE_TABLIST, g_DialogCaption, g_DialogInfo, "Выбор", "Отмена");
        }
        case DIALOGID_OBJECT_COORD: {
            ...
            ShowPlayerDialog(playerid, dialogid, DIALOG_STYLE_INPUT, "Координаты объекта", g_DialogInfo, "Выбор", "Отмена");
        }
        case DIALOGID_OBJECT_COMMENT: {
            ...
            strunpack(g_CommentString, g_ObjectData[objectid-1][OBJECT_DATA_COMMENT], MAX_COMMENT_LEN+1);
            format(g_DialogInfo, sizeof g_DialogInfo, "Текущий комментарий: %s", g_CommentString);
            ShowPlayerDialog(playerid, dialogid, DIALOG_STYLE_INPUT, "Комментарий", g_DialogInfo, "Выбор", "Отмена");
        }
        case DIALOGID_OBJECT_MATSIZE: {
            ...
            ShowPlayerDialog(playerid, dialogid, DIALOG_STYLE_TABLIST, "Размер", g_DialogInfo, "Выбор", "Отмена");
        }
        case DIALOGID_OBJECT_TEXT: {
            ...
            strunpack(g_ObjectTextString, g_ObjectText[objectid-1][materialindex], MAX_OBJECT_TEXT+1);
            format(g_DialogInfo, sizeof g_DialogInfo, "Текущий текст: %s", g_ObjectTextString);
            ShowPlayerDialog(playerid, dialogid, DIALOG_STYLE_INPUT, "Текст", g_DialogInfo, "Выбор", "Отмена");
        }
        case DIALOGID_OBJECT_FONTSIZE: {
            ...
            format(g_DialogInfo, sizeof g_DialogInfo, "Текущее значение: %i/%i", fontsize, MAX_OBJECT_FONTSIZE);
            ShowPlayerDialog(playerid, dialogid, DIALOG_STYLE_INPUT, "Размер", g_DialogInfo, "Выбор", "Отмена");
        }
            ...
            switch( dialogid ) {
                case DIALOGID_COLORALPHA_TEXTURE: {
                    ...
                    if( g_ObjectData[objectid-1][OBJECT_DATA_MATINDEX_TYPE][materialindex] == MATERIALINDEX_TYPE_TEXTURE ) {
                        new argb = g_ObjectData[objectid-1][OBJECT_DATA_MATINDEX_COLOR][materialindex];
                        format(g_DialogInfo, sizeof g_DialogInfo, "Текущие значение: %i/%i", ARGBtoA( argb ), 0xFF);
                    } else {
                        g_DialogInfo = "Текущие значение: none";
                    }
                }
