    ...
    switch(dialogid) {
            ...
            switch(listitem) {
                ...
                case LISTITEM_OBJECT_MOVE: {
                    if( g_ObjectData[objectid-1][OBJECT_DATA_ATTACH_IDTYPE] == ID_TYPE_NONE ) {
                        ...
                        SendClientMessage(playerid, RGBA_ORANGE, "Редактировать: {FFFFFF}Удерживайте ~k~~PED_SPRINT~ чтобы посмотреть по сторонам и ESC для отмены.");
                    } else {
                ...
                case LISTITEM_OBJECT_ATTACH_REMOVE: {
                    ...
                    if( new_objectid == INVALID_OBJECT_ID ) {
                        SendClientMessage(playerid, RGBA_RED, "ERROR: Этот объект не может быть откреплен");
                    }
                }
                ...
                case LISTITEM_OBJECT_MATERIALS_PASTE: {
                    ...
                    if( IsValidObject(copy_from_objectid) && copy_from_objectid != objectid ) {
                        ...
                        if( new_objectid == INVALID_OBJECT_ID ) {
                            SendClientMessage(playerid, RGBA_RED, "ERROR: Вы не может нанести текстуру на этот объект");
                        }
                    }
                }
                case LISTITEM_OBJECT_DUPLICATE: {
                    ...
                    if( new_objectid == INVALID_OBJECT_ID ) {
                        SendClientMessage(playerid, RGBA_RED, "ERROR: Вы не можеть создать копию этого объекта!");
                    } else {
                        if( !CopyObjectAttachments(objectid, new_objectid) ) {
                            SendClientMessage(playerid, RGBA_RED, "WARNING: Все аттачи не могут быть скопированы");
                        }
                ...
                case LISTITEM_OBJECT_RECREATE: {
                    new new_objectid = RecreateObject(objectid);
                    if( new_objectid == INVALID_OBJECT_ID ) {
                        SendClientMessage(playerid, RGBA_RED, "ERROR: Вы не можеть пересоздать этот объект");
                    }
                }
        ...
        case DIALOGID_OBJECT_INDEX: {
            ...
            switch(listitem) {
                case LISTITEM_OINDEX_REMOVE: {
                    ...
                    new new_objectid = RecreateObject(objectid);
                    if( new_objectid == INVALID_OBJECT_ID ) {
                        SendClientMessage(playerid, RGBA_RED, "ERROR: Вы не можеть нанести текст на этот объект");
                    }
                }
                ...
                case LISTITEM_OINDEX_MAT_COLOR_R: {
                    ...
                    if( g_ObjectData[objectid-1][OBJECT_DATA_MATINDEX_MODCOUNT] >= MAX_MATERIALINDEX_MODCOUNT ) {
                        new new_objectid = RecreateObject(objectid);
                        if( new_objectid == INVALID_OBJECT_ID ) {
                            SendClientMessage(playerid, RGBA_RED, "ERROR: Вы не может востановить цвет на этом объекте");
                        }
                    } else {
                ...
                case LISTITEM_OINDEX_TXT_ISBOLD: {
                    ...
                    if( g_ObjectData[objectid-1][OBJECT_DATA_MATINDEX_MODCOUNT] >= MAX_MATERIALINDEX_MODCOUNT ) {
                        new new_objectid = RecreateObject(objectid);
                        if( new_objectid == INVALID_OBJECT_ID ) {
                            SendClientMessage(playerid, RGBA_RED, "ERROR: Вы не можете нанести текст на этот объект");
                        }
                    } else {
                        ApplyObjectMaterialIndexData(objectid, materialindex);
                    }
        ...
        case DIALOGID_OBJECT_FONTSIZE: {
            ...
            if( sscanf(inputtext, "i", fontsize) ) {
                SendClientMessage(playerid, RGBA_RED, "ERROR: Пустое текстовое поле(только цифры)!");
                return ShowObjectDialog(playerid, dialogid), 1;
            }

            if( fontsize < 0 || fontsize > MAX_OBJECT_FONTSIZE ) {
                SendClientMessage(playerid, RGBA_RED, "ERROR: Неверный шрифт!");
                return ShowObjectDialog(playerid, dialogid), 1;
            }
        ...
        case DIALOGID_COLORALPHA_TEXTURE, DIALOGID_COLORALPHA_FONTFACE: {
            ...
            if( sscanf(inputtext, "i", alpha) && sscanf(inputtext, "h", alpha) ) {
                SendClientMessage(playerid, RGBA_RED, "ERROR: Неверное значение!");
                return ShowObjectDialog(playerid, dialogid), 1;
            }

            if( alpha < 0x00 || alpha > 0xFF ) {
                SendClientMessage(playerid, RGBA_RED, "ERROR: Вы должны ввести значение от 0 до 255!");
                return ShowObjectDialog(playerid, dialogid), 1;
            }
            ...
            if( g_ObjectData[objectid-1][OBJECT_DATA_MATINDEX_MODCOUNT] >= MAX_MATERIALINDEX_MODCOUNT ) {
                new new_objectid = RecreateObject(objectid);
                if( new_objectid == INVALID_OBJECT_ID ) {
                    SendClientMessage(playerid, RGBA_RED, "ERROR: Вы не можете установить цвет этоту объекту!");
                }
            } else {
                ApplyObjectMaterialIndexData(objectid, materialindex);
            }
